...
solmatrix[:, matrix.shape[1] -1] = matrix[:, matrix.shape[1] -1]

temp = []
for i in range(matrix.shape[0]):
    temp.append((i,matrix.shape[1]-1))
    active = activeadd(solmatrix, active, i, matrix.shape[1] -1)
    seedvalue = int(min(solmatrix, temp))
print(seedvalue, active[:400])
